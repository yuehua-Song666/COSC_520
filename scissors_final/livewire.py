...
class Livewire():
    ...
    def _get_grad_direction_cost(self, p, q):
        ...
        l = np.array([q[0]-p[0], q[1]-p[1]], float)
        if 0 not in l:
            l *= SQRT_0_5
    ...
    def _local_cost(self, p, q):
        ...
        if diagnol:
            c0 = self.cost_edges[p[0]][p[1]]-SQRT_0_5 * \
                (self.cost_edges[p[0]][p[1]]-self.cost_edges[q[0]][q[1]])
    ...
    def get_path_matrix(self, seed):
        ...
        # Accumulated cost, initialized with seed to itself
        cost = {seed: 0.0}
        paths = {}
